...
{
 public:
  PatrolAction(const Blackboard::Ptr &blackboard_ptr) :
    ...
    //gimbal_behavior = new roborts_decision::GimbalBehavior(gimbal_executor, blackboard);
    patrol_behavior = new roborts_decision::PatrolBehavior(chassis_executor, blackboard, full_path);}
  ~PatrolAction()
  {
    delete patrol_behavior;
    //delete chassis_executor;
  }
 ...
 private:
  virtual void OnInitialize() {
   ...
   virtual BehaviorState Update() {
     patrol_behavior->Run();
     std::cout<<"************************************************************************"<<std::endl;
     std::cout<<"**********patrol*****************patrol******************patrol*********"<<std::endl;
     std::cout<<"************************************************************************"<<std::endl;
  }
...
class CircleAction : public ActionNode
{
 public:
  CircleAction(const Blackboard::Ptr &blackboard_ptr) :
      ActionNode::ActionNode("Circle action", blackboard_ptr){
    std::string full_path = ros::package::getPath("roborts_decision") + "/config/decision.prototxt";
    auto blackboard_ptr_ = std::make_shared<roborts_decision::Blackboard>(full_path);
    auto chassis_executor = new roborts_decision::ChassisExecutor;
    //auto gimbal_executor = new roborts_decision::GimbalExecutor;
    auto blackboard = new roborts_decision::Blackboard(full_path);
    //gimbal_behavior = new roborts_decision::GimbalBehavior(gimbal_executor, blackboard);
    circle_behavior = new roborts_decision::CircleBehavior(chassis_executor, blackboard, full_path);}
  ~CircleAction()
  {
    delete circle_behavior;
    //delete chassis_executor;
  }
  roborts_decision::CircleBehavior* circle_behavior;
 private:
  virtual void OnInitialize() {
    ROS_INFO("%s %s", name_.c_str(), __FUNCTION__);
  };
   virtual BehaviorState Update() {
     circle_behavior->Run();
     std::cout<<"************************************************************************"<<std::endl;
     std::cout<<"**********circle*****************circle******************cirlce*********"<<std::endl;
     std::cout<<"************************************************************************"<<std::endl;
  }
